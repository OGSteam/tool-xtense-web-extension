...
function parse_galaxy_system_inserted() {
  ...
  log.info("lastAction : " + storageGetValue('lastAction', ''));
  if (storageGetValue('lastAction', '') !== 's:' + galaxy + ':' + system) {
    ...
    if (rows.snapshotLength > 0) {
      ...
      for (let i = 0; i < rows.snapshotLength; i++) {
        ...
        let player_id = Xpath.getStringValue(document, paths.player_id, row);
        if (player_id !== '') {
          player_id = player_id.trimInt();
        } else if (player) {
          player_id = XtenseMetas.getPlayerId();
        }
        let allyid = Xpath.getStringValue(document, paths.ally_id, row);
        if (allyid !== '') {
        ...
        let planet_id = Xpath.getStringValue(document, paths.planet_id, row).trimInt();
        let moon_id = Xpath.getStringValue(document, paths.moon_id, row).trimInt();
        rowsData[position] = {
          player_id: player_id,
          planet_name: name,
          planet_id: planet_id,
