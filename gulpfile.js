...
function copyBinaryFiles(source, destination) {
  return (cb) => {
    ...
    // Trouvez tous les fichiers binaires
    const files = glob.sync(source);

    files.forEach(file => {
      const relativePath = path.relative('extension', file);
      ...
      // Copier le fichier directement, sans traitement
...
function copy_files_for_browser(browser, manifest) {
  return parallel(
    ...
    () => src(["extension/**", "!extension/manifest.*", "!extension/**/*.{png,jpg,jpeg,gif,svg,ico}", "!extension/ui/chrome/**", "!extension/ui/firefox/**", "!extension/ui/edge/**", "!extension/xtense.html"])
