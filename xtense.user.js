...
function XajaxCompo(url) {
    var rcString = "";
    if (!(isOpera || isChrome)) { //Pour Firefox
        GM_xmlhttpRequest({
            method: 'GET',
            url: url || '',
            onload: function (response) {
...
function parse_rc(docrc) {
    ...
    var rslt = {};
    var date = null;
    var infos = Xpath.getOrderedSnapshotNodes(docrc, paths.list_infos);
    log(infos.snapshotLength.toString() + " list_infos elements were found");
    if (infos.snapshotLength > 0) {
        //Heure et rounds
        var rounds = Xpath.getOrderedSnapshotNodes(docrc, paths.list_rounds);
        ...
        for (var table = 0; table < infos.snapshotLength; table++) {
            ...
            //Nombre d'unités
            var values = Xpath.getOrderedSnapshotNodes(docrc, paths.list_values, info);
            ...
            var types = Xpath.getOrderedSnapshotNodes(docrc, paths.list_types, info);
            ...
            var dest = 0;
            var player = Xpath.getStringValue(docrc, paths.infos.player, info).trim();
            //Joueur non détruit
            var coords = null;
            if (player.length == 0) {
                //Dans ce cas, joueur détruit
                player = Xpath.getStringValue(docrc, paths.infos.destroyed, info).trim();
                dest = 1;
            }
            if (!dest)
            ...
            var weapons = Xpath.getStringValue(docrc, paths.infos.weapons, info).trim();
        ...
        var result = Xpath.getStringValue(docrc, paths.result).trim();
        ...
        var rounds = Xpath.getOrderedSnapshotNodes(docrc, paths.combat_round);
        var round = -1;
...
function displayOptions() {
    ...
    if (GM_getValue(prefix_GMData + 'server.check', 'false').toString() == 'true') {
        server_check += 'checked';
    }
    // Récupération des préférences  : Pages

    if (GM_getValue(prefix_GMData + 'handle.overview', 'false').toString() == 'true') {
        handle_overview += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.buildings', 'false').toString() == 'true') {
        handle_buildings += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.station', 'false').toString() == 'true') {
        handle_station += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.researchs', 'false').toString() == 'true') {
        handle_researchs += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.shipyard', 'false').toString() == 'true') {
        handle_shipyard += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.system', 'false').toString() == 'true') {
        handle_system += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.defense', 'false').toString() == 'true') {
        handle_defense += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.alliance', 'false').toString() == 'true') {
        handle_alliance += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.stats', 'false').toString() == 'true') {
        handle_stats += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.msg', 'false').toString() == 'true') {
        handle_msg_msg += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.ally', 'false').toString() == 'true') {
        handle_msg_ally += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.spy', 'false').toString() == 'true') {
        handle_msg_spy += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.ennemy.spy', 'false').toString() == 'true') {
        handle_msg_ennemy_spy += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.rc', 'false').toString() == 'true') {
        handle_msg_rc += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.rc.cdr', 'false').toString() == 'true') {
        handle_msg_rc_cdr += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.expeditions', 'false').toString() == 'true') {
        handle_msg_expeditions += 'checked';
    }
    if (GM_getValue(prefix_GMData + 'handle.msg.commerce', 'false').toString() == 'true') {
...
function initParsers() {
    ...
    Xpath = {
        //node est facultatif
        getNumberValue: function (document, xpath, node) {
            node = node ? node : document;
            return document.evaluate(xpath, node, null, XPathResult.NUMBER_TYPE, null).numberValue;
        },
        getStringValue: function (document, xpath, node) {
            node = node ? node : document;
            return document.evaluate(xpath, node, null, XPathResult.STRING_TYPE, null).stringValue;
        },
        ...
        getSingleNode: function (document, xpath, node) {
            node = node ? node : document;
